...
class UnitTestRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            source_folder (str):
                Source folder within the package folder.  This is for code coverage.
            failfast (bool):
        ...
        self.source_folder = self._get_source_folder(package_folder, source_folder)
    ...
    @staticmethod
    def _get_source_folder(package_folder, source_folder):
        ...
        return os.path.join(package_folder, source_folder)
    ...
    def percent(self) -> float:
        ...
        Returns:
            float:
                Code coverage as a percent, or `None` if no coverage was done.
        """
    ...
    def list_tests(self) -> List[str]:
        ...
        with self._mutex:
            if self._suite is None:
                return None
            id_list = get_test_id_list(self._suite)
        ...
        with self._mutex:
            with path_context(self.package_folder):
                ...
                if not self.no_cover:
                    self._coverage = Coverage(source=[self.source_folder])
    ...
    def coverage_report(self):
        """
        Write the coverage report to the console.
        """

        self._coverage.report()

    def write_results(
        self, output_folder=None, coverage_prefix=None, unittest_prefix=None
    ) -> None:
        """
        ...
        with self._mutex:
            ...
            if self._coverage:
                self._percent = self._coverage.xml_report(outfile=coverage_xml)
            ...
            if self._results:
                with open(results_json, mode="w", encoding="UTF-8") as f:
                    json.dump(
                        obj=to_jobj(self._results), fp=f, indent=4, sort_keys=True
                    )
