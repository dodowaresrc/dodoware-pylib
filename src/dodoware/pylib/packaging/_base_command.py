...
class BaseCommand(Command, metaclass=abc.ABCMeta):
    ...
    def _run_module(self, module_name, module_args=None, output_folder=None):
        ...
        return process_runner.get_status()
    ...
    def add_folder_args(self, output=False, build=False, source=False, test=False):
        ...
        self.parser.add_argument(
            "--package_folder", help="python package folder (default is current folder)"
        )

        if output:
            self.parser.add_argument(
                "--output_folder", help="output folder (default='.output')"
            )

        if build:
            self.parser.add_argument(
                "--build_folder", help="build folder (default='.build')"
            )

        if source:
            self.parser.add_argument(
                "--source_folder", help="source folder (default='src')"
            )

        if test:
            self.parser.add_argument(
                "--test_folder", help="test folder (default='test')"
            )
